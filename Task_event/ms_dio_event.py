...
from Datajoint_nwb_utils.datajoint_table_helpers import insert_analysis_table_entry, fetch1_dataframe, \
    get_table_object_id_name
from Utils.df_helpers import df_pop
from Utils.list_helpers import return_n_empty_lists
from Datajoint_nwb_utils.nwbf_helpers import get_nwb_file, events_in_epoch_bool
...
class ProcessedDioEvents(dj.Computed):
    ...
    class Stim(dj.Part):
        ...
        def fetch1_dataframe(self):
            return fetch1_dataframe(self, object_name="stim").set_index("dio_event_times")
    ...
    class StimUp(dj.Part):
        ...
        def fetch1_dataframe(self):
            return fetch1_dataframe(self, object_name="stim_up").set_index("dio_event_times")
    ...
    class StimDown(dj.Part):
        ...
        def fetch1_dataframe(self):
            return fetch1_dataframe(self, object_name="stim_down").set_index("dio_event_times")
    ...
    def make(self, key):
        ...
        poke_dio_names = [dio_name for dio_name in dio_names if "stim" in dio_name]  # get names of stim dio events
        dio_stim_df = all_dio_df[all_dio_df["dio_event_names"].isin(poke_dio_names)]
        dio_stim_ups_df = dio_stim_df[dio_stim_df["dio_event_values"] == 1]  # filter for up events
        dio_stim_downs_df = dio_stim_df[dio_stim_df["dio_event_values"] == 0]  # filter for down events
        ...
        insert_analysis_table_entry(self.StimUp(), [dio_stim_ups_df], key, reset_index=True)

        # Populate subtable for stim down events
        insert_analysis_table_entry(self.StimDown(), [dio_stim_downs_df], key, reset_index=True)

        # Populate subtable for well pokes
        ProcessedDioEvents.Pokes.insert1({**key, **{"dio_poke_names": dio_pokes_df["dio_event_names"].to_numpy(),
                                                    "dio_poke_times": dio_pokes_df.index.to_numpy(),
                                                    "dio_poke_values": dio_pokes_df["dio_event_values"].to_numpy()}})
        print('Populated ProcessedDioEvents.Pokes for file {nwb_file_name}, epoch {epoch}'.format(**key))
        ...
        print('Populated ProcessedDioEvents.FirstPokes for file {nwb_file_name}, epoch {epoch}'.format(**key))
        ...
        print('Populated ProcessedDioEvents.LastDownPokes for file {nwb_file_name}, epoch {epoch}'.format(**key))
        ...
        ProcessedDioEvents.Pumps.insert1({**key, **{"dio_pump_names": dio_pumps_df["dio_event_names"].to_numpy(),
                                                    "dio_pump_times": dio_pumps_df.index.to_numpy(),
                                                    "dio_pump_values": dio_pumps_df["dio_event_values"].to_numpy()}})
        print('Populated ProcessedDioEvents.Pumps for file {nwb_file_name}, epoch {epoch}'.format(**key))
    ...
    def fetch1_dataframe(self):
        return fetch1_dataframe(self, object_name="processed_dio_events").set_index("dio_event_times")
