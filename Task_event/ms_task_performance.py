...
@schema
class AlternationTaskPerformanceSel(dj.Manual):
    definition = """
    ...
    def insert_defaults(self):
        keys = [x for x in ProcessedDioEvents().fetch("KEY") if
                (TaskIdentification & x).fetch1("contingency") == "wtrack"]
        for key in keys:
            self.insert1(key, skip_duplicates=True)


@schema
class AlternationTaskPerformance(dj.Computed):
    definition = """
    ...
    def make(self, key):
        ...
        well_pokes = (ProcessedDioEvents.FirstUpPokes() & key).fetch1("dio_first_poke_names")
        well_pokes = [get_poke_name(x) for x in well_pokes]  # replace with more informative name (e.g. poke3 --> well name)
        well_pokes = [x.replace("poke", "well") for x in well_pokes]  # replace poke with well
        ...
        reward_outcomes, performance_outcomes = zip(
            *[(AlternationTaskRule & {
                "previous_side_well": previous_side_well, "previous_well": previous_well,
                "current_well": current_well}).fetch1("reward_outcome", "performance_outcome")
              for previous_side_well, previous_well, current_well
              in zip(previous_side_wells, previous_wells, current_wells)])

        # Populate parent table
        check_lists_same_length([previous_side_wells, previous_wells, current_wells,
                                 reward_outcomes, performance_outcomes])
        insert1_print(self,
