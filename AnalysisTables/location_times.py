...
from spyglass.common import IntervalList, interval_list_intersect, interval_list_contains_ind
...
class LocationTimes(SpyglassMixin, dj.Computed):
    ...
    def make(self, key):
        ...
        contained = np.array([self._in_location(pos_df.position_x, pos_df.position_y, center, radius)
                              for center, radius in zip(centers, radii)]).any(axis=0).astype(int)
        ...
        intervals = np.array([[time[i_enter],time[i_exit]] for i_enter, i_exit in zip(enter_times, exit_times)])
    ...
    def validate_location(self, key=dict()):
        ...
        fig, ax = plt.subplots()
        contained_indices = interval_list_contains_ind(contained_intervals,pos_df.index.values,)
        plt.scatter(pos_df.position_x, pos_df.position_y, c='cornflowerblue', s=10)
        plt.scatter(pos_df.position_x.iloc[contained_indices], pos_df.position_y.iloc[contained_indices], c='firebrick', s=10)
        circles = [plt.Circle(center, radius, color='red', fill=True,alpha=.3) for center, radius in zip(centers, radii)]
