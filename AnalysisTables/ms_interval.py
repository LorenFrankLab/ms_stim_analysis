...
@schema
class EpochIntervalListName(dj.Computed):
    definition = """
    ...
    def make(self, key):
        ...
        # *** HARD CODED VALUES ***
        epsilon = .11  # tolerated time difference in epoch boundaries across epoch/pos intervals
        # *************************
        ...
        epoch_pos_valid_time_dict = {epoch: [] for epoch in
                                     epochs}  # store correspondence between epoch number and pos x valid time interval names
        for epoch, epoch_interval_list_name in zip(epochs, epoch_interval_list_names):  # for each epoch
            ...
            epoch_time_interval_widened = np.asarray([epoch_time_interval[0] - epsilon,
                                                      epoch_time_interval[
                                                          1] + epsilon])  # widen to tolerate small differences in epoch boundaries across epoch/pos intervals
